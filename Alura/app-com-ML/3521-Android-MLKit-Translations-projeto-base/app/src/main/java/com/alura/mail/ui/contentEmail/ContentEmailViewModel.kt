...
class ContentEmailViewModel @Inject constructor(
    ...
    private fun identifyEmailLanguage() {
        _uiState.value.selectedEmail?.let { email ->
            textTranslator.languageIdentifier(text = email.content, onSuccess = { langName ->
                _uiState.value = _uiState.value.copy(
                    languageIdentified = langName
                )
    ...
    private fun downloadDefaultLanguageModel() {
        val defaultLang = _uiState.value.localLanguage?.code.toString()
        textTranslator.downloadModel(
            modelName = defaultLang,
            onSuccess = {
    ...
    fun tryTranslateEmail() {
        with(_uiState.value) {
            if (translatedState == TranslatedState.TRANSLATED) {
                ...
                val languageIdentifier = _uiState.value.languageIdentified?.code.toString()
                textTranslator.verifyDownloadModel(
                    modelCode = languageIdentifier,
                    onSuccess = { translateEmail() },
                    onFailure = {
                        _uiState.value = _uiState.value.copy(
                ...
                languageIdentified?.let { languageIdentified ->
                    localLanguage?.let { localLanguage ->
                        textTranslator.textTranslate(
                            ...
                            onSuccess = {
                                _uiState.value = _uiState.value.copy(
                                    email.copy(content = it),
                                    translatedState = TranslatedState.TRANSLATING
                                )
                                translateSubject()
                            },
                            onFailure = {
