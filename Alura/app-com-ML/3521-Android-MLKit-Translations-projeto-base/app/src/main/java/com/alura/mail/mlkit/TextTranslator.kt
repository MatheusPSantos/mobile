...
class TextTranslator(private val fileUtil: FileUtil) {
    fun languageIdentifier(
        text: String, onSuccess: (Language) -> Unit = {}, onFailure: () -> Unit = {}
    ) {
        ...
        languageIdentifier.identifyLanguage(text).addOnSuccessListener { languageCode ->
            ...
            if (languageName != LanguageIdentifier.UNDETERMINED_LANGUAGE_TAG) {
                onSuccess(Language(name = languageName, code = languageName))
            } else {
                onFailure()
            }
    ...
    fun downloadModel(
        ...
        val modelManager = RemoteModelManager.getInstance()
        modelManager.download(model, conditions).addOnSuccessListener {
            onSuccess()
        }.addOnFailureListener {
            onFailure()
        }
            ...
            .addOnSuccessListener { models ->
                ...
                models.forEach { model ->
                    try {
                        ...
                        Log.e("getDownloadModels", "error: $e")
                    }
                }
